...
func resourceHarborProjectMemberCreate(d *schema.ResourceData, meta interface{}) error {
	...
	err = harborProjectMemberUpdate(d, foundMember)
	if err != nil {
		return err
	}
...
func harborProjectMemberUpdate(d *schema.ResourceData, r *apimodels.ProjectMemberEntity) error {
	d.SetId(fmt.Sprintf("%d/%d", r.ProjectID, r.ID))
	attributes := map[string]interface{}{
		"role_id":    r.RoleID,
		"group_name": r.EntityName,
	}
	var err error
...
func resourceHarborProjectMemberRead(d *schema.ResourceData, meta interface{}) error {
	...
	err = harborProjectMemberUpdate(d, resp.Payload)
	if err != nil {
		return err
	}
